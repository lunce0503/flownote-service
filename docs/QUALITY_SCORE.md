...
| 영역 | 기본 게이트 | 상태 기록 방식 |
| --- | --- | --- |
| Vite React | `cd flownote && yarn verify`, `yarn e2e` | lint+typecheck+build와 캔버스 저장 브라우저 스모크 기록 |
...
| Go services | `cd flownote-canvas && go test ./...`, `cd flownote-serve && go test ./...` | 인증·JSON 오류 계약 테스트 포함. 로컬 Go가 없으면 Go 1.23 컨테이너로 실행 |
| Expo mobile | `cd flownote-mobile && yarn verify` | lint, typecheck, 정적 서버 테스트, Expo web export 확인 |
...
## 최근 해소 (2026-08-21 주요 부채 정리)

- **웹 정적 게이트**: TypeScript와 ESLint 오류를 0으로 정리하고 CI 필수 단계로 전환했다.
- **런타임 회귀 검증**: 로그인→캔버스 선 입력→Socket.IO 저장 ack Playwright 스모크를 추가했다.
- **백엔드 테스트 기반**: Go 인증/오류 응답, Spring Redis 장애 DB 폴백/관리자 권한, AI 인덱스 격리와 capability 테스트를 추가했다. 두 Go 모듈의 `go.sum`도 생성했다.
- **AI 부팅 격리**: Gemini 비밀값이 없어도 health가 부팅되고, Ollama 내부망 기능 가용성을 capability 계약으로 노출한다.
...
- 여러 서비스가 같은 PostgreSQL을 공유하므로 마이그레이션 변경은 Spring Flyway, Go 소비자, 웹·모바일 호출부를 함께 확인해야 한다.

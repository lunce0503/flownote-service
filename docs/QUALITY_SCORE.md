...
- 데이터 안정성: migration 순서, 기본값, 기존 데이터 호환성을 확인한다.
- UX 완결성: loading, empty, error, success 상태가 있다.
...
- `docs/generated/db-schema.md`는 수동 작성 문서이므로 새 migration 이후 드리프트가 생길 수 있다.
- 여러 서비스가 같은 PostgreSQL을 공유하므로 migration 변경은 Spring, Next, React 호출부를 함께 확인해야 한다.

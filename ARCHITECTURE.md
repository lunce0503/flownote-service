...
| `flownote-API/` | FastAPI 서비스 · **API 게이트웨이** | 클라이언트 `/api/**`를 받아 백엔드로 라우팅(canvas→Go, 코어→Spring). 자체 백엔드: 메인 에이전트(Gemini), 실시간 캔버스 소켓 중계, 에이전트 노트(내부망 Ollama), 주식 시세 |
...
- API 게이트웨이: `flownote-API`가 클라이언트 `/api/**`를 받아 경로로 백엔드를 정한다. `/api/canvas/**`→`CANVAS_API_BASE_URL`(Go), 그 외 코어→`CORE_API_BASE_URL`(Spring). FastAPI 자체 백엔드(AI `/api/aiclient`, 에이전트 노트, 주식 `/api/market`, 채팅, 소셜)는 각 라우터가 먼저 매칭되어 로컬 처리되고, 게이트웨이 catch-all은 마지막에 나머지를 프록시한다.
- 실시간 캔버스는 React ↔ FastAPI Socket.IO로 연결되고, FastAPI가 스트로크/이미지/텍스트를 `CANVAS_API_BASE_URL`(Go)의 캔버스 API에 중계·저장한다.
- 라우팅 지점: 프론트 `VITE_CANVAS_API_URL`(캔버스 HTTP), FastAPI `CANVAS_API_BASE_URL`(소켓 중계 + 게이트웨이). 둘 다 flownote-canvas(Go)를 가리킨다.

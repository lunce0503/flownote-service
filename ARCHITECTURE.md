...
| `flownote-API/` | **API 게이트웨이**(FastAPI) | 단일 진입점: `/api/**`·`/uploads/**`를 경로로 라우팅(캔버스·노트→Go, 부가기능→serve, AI→flownote-ai, 인증→Spring) + 캔버스 Socket.IO 중계 |
| `flownote-canvas/` | **Go 필기 백엔드** | 캔버스(`/api/canvas/**`) + 노트(`/api/notes`·`/api/note-folders`·업로드) 전담. Postgres/S3 공유 |
| `flownote-serve/` | **Go 부가기능 백엔드** | `/api/{schedule-items,tasks,stocks,social,chat}` 전담. 게이트웨이 뒤 사설, 서버리스(App Sleeping) |
| `flownote-ai/` | **AI 백엔드**(FastAPI) | `/api/aiclient`(Gemini), `/api/agent-note`(내부망 Ollama), `/api/market`(시세, Redis 캐시). 코어 호출은 게이트웨이 경유 |
| `flownote-server/` | **인증 서버**(Spring) | 가입/로그인·사용자·모바일 설정·`app_sessions` 발급만. 도메인 코드 전부 이관 완료, 서버리스(App Sleeping)로 비용 축소 |
...
| `redis` (Redis) | 캐시 · :6379 | **세션 캐시**(전 백엔드 공용, TTL 5분) · 시세 캐시(flownote-ai) · socket.io 매니저(게이트웨이) |
...
- 인증(계정·세션 발급)의 기준은 `flownote-server`(인증 서버)다. 도메인 데이터는 각 소유 서버가 담당한다: 필기(캔버스·노트)=flownote-canvas, 부가기능=flownote-serve.
- React 웹 앱의 `VITE_CORE_API_URL`은 **게이트웨이**를 가리킨다(단일 진입점). 게이트웨이가 경로로 백엔드를 분배하므로 프론트는 이관을 모른다.
...
- API 게이트웨이 라우팅: `/api/{canvas,notes,note-folders,upload,admin}`·`/uploads/**`→`CANVAS_API_BASE_URL`(Go), `/api/{schedule-items,tasks,stocks,social,chat}`→`SERVE_API_BASE_URL`(flownote-serve), `/api/{aiclient,agent-note,market}`→`AI_API_BASE_URL`, 그 외(인증·모바일)→`CORE_API_BASE_URL`(Spring). flownote-ai의 코어 호출(MCP 도구 포함)도 게이트웨이를 경유해 같은 규칙을 탄다.
...
- AI 백엔드는 `flownote-ai`가 담당한다: 메인 플래너 에이전트(외부 Gemini, MCP 도구의 최종 저장은 게이트웨이 경유), 에이전트 노트(내부망 Ollama — 클라우드에선 미동작), 주식 시세(`/api/market`, Redis 캐시 — flownote-serve가 `STOCK_MARKET_DATA_URL`로 소비).
- 서버리스 운영: `flownote-serve`·`flownote-main`은 App Sleeping으로 유휴 시 잠들고 게이트웨이의 콜드스타트 재시도가 첫 요청을 웨이크한다(수 초 지연 허용이 전제). 상시 가동이 필요해지면 해당 서비스만 Sleeping 해제.
...
- Redis 사용처: ① 세션 캐시 `session:{token}`(전 백엔드, TTL 5분, 장애 시 DB 폴백) ② flownote-ai 시세 캐시 ③ 게이트웨이 socket.io Redis 매니저(replica 확장 대비). 세션 캐시는 로그아웃 엔드포인트가 없어 만료 후 최대 5분 오차를 허용한다.
...
| `flownote-server/` | Railway `flownote-main` | **인증 전용 + App Sleeping**, health `/actuator/health` |
| `flownote-canvas/` (Go) | Railway `flownote-canvas` | `/api/canvas/**`·관리자 진단 전담, 변수는 flownote-main 참조, PORT=8080(로컬 8090) |
| `flownote-serve/` (Go) | Railway `flownote-serve` | 사설 :8095, 게이트웨이 뒤 요청 시 웨이크(**App Sleeping**), 공개 도메인 없음 |

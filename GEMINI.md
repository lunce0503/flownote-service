...
6. 기록: 버그와 실행 로그는 `logs/`, 분석 산출물과 작업 결과는 `logs/report/`, 재사용 가능한 프로젝트 컨벤션은 `.codex/memories/`에 남긴다.
7. Docker 빌드 및 실행: 작업 종료 전 저장소 루트에서 `docker compose up -d --build`를 실행해 통합 빌드와 백그라운드 실행을 확인한다.
8. 보고: 최종 응답에는 하위 프로젝트 검증 결과와 `docker compose up -d --build` 결과를 함께 포함한다.

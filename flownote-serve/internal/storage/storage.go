...
// Delete는 키의 오브젝트를 삭제한다(없어도 오류로 보지 않는다 — Spring CanvasAssetStorage와 동일).
func (s *Store) Delete(ctx context.Context, objectKey string) error {
	if !s.configured || strings.TrimSpace(objectKey) == "" {
		return nil
	}
	...
	if err != nil {
		return httpjson.Errorf(http.StatusBadGateway, "Object storage 삭제에 실패했습니다.")
	}
	return nil
}
